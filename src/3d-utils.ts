...
import { SCREEN_WIDTH, SCREEN_HEIGHT, HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, FOCAL_LENGTH } from './constants';
...
export function create3DPolygonFromJSONData(jsonData: Polygon3DData): Polygon3D {
    // Convert plain objects to Point3D instances
    const points = jsonData.points.map(point => new Point3D(point.x, point.y, point.z));
...
export function create3DObjectFromJSONData(jsonDataArray: Polygon3DData[]): Object3D {
    // Convert each JSON polygon to Polygon3D
    const polygons = jsonDataArray.map(polygonData => create3DPolygonFromJSONData(polygonData));
...
export function convert3DPolygonTo2DPolygon(polygon3D: Polygon3D): Polygon2D {
    ...
    const projectedPoints = polygon3D.getPoints().map(point => project3DPoint(point));
...
export function rotate3DObjectY(object3D: Object3D, angleInDegrees: number): void {
...
export function draw3DObject(ctx: CanvasRenderingContext2D, object3D: Object3D): void {
